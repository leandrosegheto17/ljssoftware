...
| G-02 | Header/Nav e Footer/Contato devem permanecer byte-idênticos entre **todas** as páginas HTML do site (lista viva em `TASK.md` Seção 1.1 — hoje 7 arquivos: `index.html`, `apps.html`, `sobre.html`, `evolucao-segura.html`, `destino-ideal.html`, `radar-esportivo.html`, `404.html`; URLs públicas sem extensão, regra vigente após a RL5.1: `/` (index), `/apps`, `/[slug]` — o Cloudflare Pages redireciona 308 de `.html` para a URL limpa, e os nomes de arquivo continuam `.html`; nas páginas de app, `aria-current="page"` recai no item "Apps"; a única variação permitida é o atributo `aria-current="page"` no item de nav correspondente, incl. no item-mãe "Apps" para páginas de produto sem item de nav próprio, `UX-SPEC.md` Seção 7) | SDD.md, RT-02 / TASK.md Seção 1 | Divergência visual/funcional entre páginas; corrigir replicando o bloco correto em todas as páginas na mesma tarefa |
...
| G-15 | **[Revisada 2026-09-18, RA-10/RN-05]** Todo app publicado, web ou desktop, roteia o card da vitrine (`apps.html`, URL `/apps`)/prévia (`index.html`, URL `/`) para a página interna, cuja URL pública é `/[slug]` (sem extensão; arquivo `[slug].html`); a URL externa do app vive só no CTA "Abrir app" dentro dessa página; nunca link direto para binário ou arquivo de download (o CTA de download de app desktop também fica só na página) | PRD-TECNICO.md RN-03 / SDD.md Seção 5 (modelo de dados, campo `tipo`) | Contraria requisito de produto confirmado pelo stakeholder; corrigir trocando o `href` do badge/link para a página própria, nunca para o arquivo |
...
| 2026-09-18 | gestor (PRD 9.5 / RN-05) | gestor (chapéu CTO) | G-15 revisada (de "app desktop roteia para página própria" para "todo app publicado, web ou desktop, roteia para `[slug].html`; URL externa só no CTA 'Abrir app'; nunca link direto para binário"); G-02 ampliada de 5 para 7 páginas (+`destino-ideal.html`, +`radar-esportivo.html`) e `aria-current="page"` em "Apps" nas páginas de app | Lote 12 / RA-10 aprovado pelo usuário, com mudança de estratégia de roteamento; substitui o critério "web = link direto" de `PRD.md` 8.2 | Permanente (mudança estrutural) |
| 2026-09-18 | gestor (chapéu CTO) | usuário (2026-09-18) | G-17 esclarecida ("dado 100% fictício" = dado sensível de paciente/cliente/terceiro; dado público real, ex.: resultados esportivos, e nome do produto capturado são permitidos); G-02/G-15 (e `PRD.md` 8.2, `PRD-TECNICO.md` Seção 9) atualizadas para URLs públicas sem extensão (`/`, `/apps`, `/[slug]`), mantendo nomes de arquivo `.html` | Decisão RL12.2 (`CTO-REVIEW.md`) e regra vigente após a RL5.1: Cloudflare Pages redireciona 308 de `.html` para a URL limpa | Permanente (mudança estrutural) |
